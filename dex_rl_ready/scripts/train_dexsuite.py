"""Dexsuite Kuka+Allegro dexterous lift RL training (SKRL PPO)."""
...
log_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "logs", "dexsuite"))
os.makedirs(log_dir, exist_ok=True)
agent_cfg.experiment.directory = log_dir
agent_cfg.experiment.experiment_name = "dexsuite_lift"
agent_cfg.experiment.checkpoint_interval = 500

print(f"[INFO] Training {args_cli.train_iters} iterations")
print(f"[DEBUG] obs_space: {env.observation_space.shape if hasattr(env.observation_space, 'shape') else 'Dict'}, act_space: {env.action_space.shape if hasattr(env.action_space, 'shape') else 'Dict'}")

obs_space = env.observation_space
if hasattr(obs_space, 'spaces'):
    total_obs_dim = sum(s.shape[-1] for s in obs_space.spaces.values())
elif hasattr(obs_space, 'shape'):
    total_obs_dim = obs_space.shape[-1]
else:
    total_obs_dim = obs_space["policy"].shape[-1] if isinstance(obs_space, dict) else 256

print(f"[DEBUG] Total observation dim: {total_obs_dim}")
